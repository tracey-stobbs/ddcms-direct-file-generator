...
describe('MCP Common tools', () => {
    ...
    it('preview_file_name predicts name and columnCount', async () => {
        const res = await Common.previewFileName({
            fileType: 'SDDirect',
            sun: '123456',
            includeHeaders: true,
        });
    ...
    it('validate_processing_date normalizes formats', async () => {
        const ok = await Common.validateProcessingDate({
            fileType: 'EaziPay',
            date: '2025-12-24',
            dateFormat: 'YYYY-MM-DD',
        });
        expect(ok.valid).toBe(true);
        const bad = await Common.validateProcessingDate({
            fileType: 'SDDirect',
            date: '2025-13-40',
        });
    ...
    it('list_output_files and read_output_file work with safe paths', async () => {
        ...
        const read = await Common.readOutputFile({
            fileType: 'SDDirect',
            sun: '111111',
            fileName: 'test.csv',
            mode: 'utf8',
