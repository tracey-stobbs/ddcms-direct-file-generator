...
import { validateRequest, validateAndNormalizeHeaders, validateAndNormalizeRequest } from "./requestValidator.js";
...
describe("Request Validator", () => {
  describe("validateRequest", () => {
    ...
    it("should validate fileType values", () => {
      ...
      expect(errors).toContain("fileType must be one of: SDDirect, Bacs18PaymentLines, Bacs18StandardFile, EaziPay");
    });

    it("should validate numberOfRows if provided", () => {
      const errors1 = validateRequest({ fileType: "SDDirect", canInlineEdit: true, numberOfRows: -5 });
      expect(errors1).toContain("numberOfRows must be a positive integer");

      const errors2 = validateRequest({ fileType: "SDDirect", canInlineEdit: true, numberOfRows: 0 });
      expect(errors2).toContain("numberOfRows must be a positive integer");

      const errors3 = validateRequest({ fileType: "SDDirect", canInlineEdit: true, numberOfRows: 1.5 });
      expect(errors3).toContain("numberOfRows must be a positive integer");

      const errors4 = validateRequest({ fileType: "SDDirect", canInlineEdit: true, numberOfRows: 15 });
  ...
  describe("validateAndNormalizeHeaders", () => {
    it("should preserve includeHeaders for supported file types", () => {
      ...
      const request2: Request = { fileType: "Bacs18StandardFile", canInlineEdit: true, includeHeaders: true };
      const normalized2 = validateAndNormalizeHeaders(request2);
      expect(normalized2.includeHeaders).toBe(true);

      const request3: Request = { fileType: "SDDirect", canInlineEdit: true, includeHeaders: false };
    ...
    it("should override includeHeaders to false for unsupported file types", () => {
      ...
      const request2: Request = { fileType: "Bacs18PaymentLines", canInlineEdit: true, includeHeaders: true };
      const normalized2 = validateAndNormalizeHeaders(request2);
