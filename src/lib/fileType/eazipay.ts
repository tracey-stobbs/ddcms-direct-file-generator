...
export function generateValidEaziPayRow(
  ...
  return {
    ...
    destinationAccountNumber: faker.finance.accountNumber({ length: 8 }),
  ...
  for (const fieldName of fieldsToInvalidate) {
    ...
    switch (fieldName) {
      case 'transactionCode':
        row.transactionCode = generateInvalidFieldValue(fieldName, row.transactionCode) as string;
        break;
...
function getOriginatingDetails(request: Request): {
  ...
  return {
    ...
    accountNumber: details?.accountNumber || faker.finance.accountNumber({ length: 8 }),
...
function generatePaymentReference(): string {
  let ref = faker.string.alphanumeric({ length: faker.number.int({ min: 7, max: 17 }) });
  ...
  while (
    ...
    ref = faker.string.alphanumeric({ length: faker.number.int({ min: 7, max: 17 }) });
  }
...
function generateSunNumber(transactionCode: string): string | undefined {
  ...
  if (faker.number.int({ min: 0, max: 1 }) === 1) {
    return faker.string.alphanumeric({ length: faker.number.int({ min: 5, max: 10 }) });
  }
...
function generateInvalidFieldValue(fieldName: string, transactionCode: string): string | number {
  switch (fieldName) {
    ...
    case 'bacsReference':
      return 'DDIC' + faker.string.alphanumeric({ length: 5 }); // Starts with DDIC (invalid)
    ...
    case 'sunNumber':
      ...
      if (!EaziPayValidator.isSunNumberAllowed(transactionCode)) {
        return faker.string.alphanumeric({ length: 5 }); // Invalid because not allowed
      }
...
/**
 * Validate and normalize an EaziPay processing date string.
 * Reuses DateFormatter for format validation and normalization.
 */
export function validateEaziPayDate(
  date: string,
  opts?: { dateFormat?: string }
): { valid: true; normalized: string; warnings?: string[] } | { valid: false; errors: string[] } {
  let fmt: EaziPayDateFormat;
  try {
    fmt = DateFormatter.parseEaziPayFormat(opts?.dateFormat ?? 'YYYY-MM-DD');
  } catch {
    return { valid: false, errors: ['Unsupported dateFormat'] };
  }

  const luxonFormat = fmt === 'YYYY-MM-DD' ? 'yyyy-MM-dd' : fmt === 'DD-MMM-YYYY' ? 'dd-MMM-yyyy' : 'dd/MM/yyyy';
  const parsed = DateTime.fromFormat(date, luxonFormat);
  if (!parsed.isValid) return { valid: false, errors: ['Invalid date for format'] };

  const normalized = DateFormatter.formatEaziPayDate(parsed, fmt);
