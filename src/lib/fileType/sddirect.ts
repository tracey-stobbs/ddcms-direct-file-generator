...
function getOriginatingDetails(request: Request): Record<string, string> {
  ...
  return {
    ...
    [SDFields.OriginatingAccountNumber]: details?.accountNumber || faker.finance.accountNumber(8),
    [SDFields.OriginatingAccountName]: details?.accountName || faker.person.fullName().slice(0, 18),
  };
}
...
export function generateValidSDDirectRow(request: Request): Record<string, unknown> {
  ...
  return {
    ...
    [SDFields.TransactionCode]: faker.helpers.arrayElement(["01", "17", "18", "99", "0C", "0N", "0S"]),
    [SDFields.RealtimeInformationChecksum]: faker.helpers.arrayElement(["/" + faker.string.alpha({ length: 3, casing: "upper" }), "0000", ""]),
...
function generateInvalidValue(field: string): string {
  switch (field) {
  case SDFields.DestinationAccountName: return faker.string.alpha({ length: 25 }); // too long
  case SDFields.DestinationSortCode: return faker.string.alpha({ length: 6 }); // not numeric
  case SDFields.DestinationAccountNumber: return faker.string.alpha({ length: 8 }); // not numeric
  case SDFields.PaymentReference: return "DDIC" + faker.string.alphanumeric(5); // starts with DDIC
  case SDFields.Amount: return "-9999.99"; // negative
  case SDFields.TransactionCode: return "XX"; // invalid code
  case SDFields.RealtimeInformationChecksum: return "////"; // invalid pattern
  case SDFields.PayDate: return "20250101"; // not 3+ working days in future
  case SDFields.OriginatingSortCode: return "ABCDEF"; // not numeric
  case SDFields.OriginatingAccountNumber: return "ABCDEFGH"; // not numeric
  case SDFields.OriginatingAccountName: return faker.string.alpha({ length: 25 }); // too long
...
export function generateInvalidSDDirectRow(request: Request): Record<string, unknown> {
  ...
  if (request.defaultValues?.originatingAccountDetails?.canBeInvalid) {
    const origFields = [SDFields.OriginatingSortCode, SDFields.OriginatingAccountNumber, SDFields.OriginatingAccountName];
...
export const sdDirectAdapter: FileTypeAdapter = {
  ...
  buildRow(params) {
    ...
    const data = params.validity === "invalid" ? generateInvalidSDDirectRow(req) : generateValidSDDirectRow(req);
