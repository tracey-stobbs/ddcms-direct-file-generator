...
function getOriginatingDetails(request: Request): {
    'Originating Sort Code': string;
    'Originating Account Number': string;
    'Originating Account Name': string;
} {
    ...
    return {
        ...
        'Originating Account Number':
            details?.accountNumber || faker.finance.accountNumber({ length: 8 }),
...
export function generateValidSDDirectRow(request: Request): Record<string, unknown> {
    ...
    return {
        ...
        'Destination Account Number': faker.finance.accountNumber({ length: 8 }),
        ...
        'Realtime Information Checksum': faker.helpers.arrayElement([
...
function generateInvalidValue(field: string): string {
    switch (field) {
        case 'Destination Account Name':
            return faker.string.alpha({ length: 25 }); // too long
        case 'Destination Sort Code':
        ...
        case 'Payment Reference':
            return 'DDIC' + faker.string.alphanumeric({ length: 5 }); // starts with DDIC
        case 'Amount':
            return '-9999.99'; // negative
        case 'Transaction code':
            return 'XX'; // invalid code
        case 'Realtime Information Checksum':
            return '////'; // invalid pattern
        case 'Pay Date':
            return '20250101'; // not 3+ working days in future
        case 'Originating Sort Code':
            return 'ABCDEF'; // not numeric
        case 'Origininating Account Number':
            return 'ABCDEFGH'; // not numeric (intentionally invalid key to simulate error)
        case 'Originating Account Number':
            return 'ABCDEFGH'; // not numeric
        case 'Originating Account Name':
            return faker.string.alpha({ length: 25 }); // too long
        default:
            return 'INVALID';
    }
...
/**
 * Validate and normalize an SDDirect date (expects YYYYMMDD).
 */
export function validateSDDirectDate(
    date: string,
): { valid: true; normalized: string } | { valid: false; errors: string[] } {
    const m = date.match(/^\d{8}$/);
    if (!m) return { valid: false, errors: ['Expected YYYYMMDD'] };
    const dt = DateTime.fromFormat(date, 'yyyyLLdd');
    if (!dt.isValid) return { valid: false, errors: ['Invalid date'] };
    return { valid: true, normalized: dt.toFormat('yyyyLLdd') };
}
