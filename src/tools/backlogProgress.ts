...
export function parseBacklogMarkdown(md: string): {
    epics: EpicCounts[];
    overall: LightCounts;
    openQuestions: LightCounts;
} {
    ...
    for (const rawLine of lines) {
        ...
        if (epicHeader) {
            ...
            if (!epicMap.has(key)) {
                epicMap.set(key, {
                    code: epicHeader.code,
                    title: epicHeader.title,
                    counts: cloneZero(),
                });
            }
...
export function renderDashboard(
    ...
    lines.push('|:--|:--:|:--:|:--:|:--:|');
    const row = (scope: string, c: LightCounts): string =>
        `| ${scope} | ${c.green} | ${c.yellow} | ${c.red} | ${c.white} |`;
...
export function resolveBacklogPath(cwd: string, relative?: string): string {
    ...
    return path.resolve(
        ...
        'documentation',
        'Phase 2 - MCP Server',
        'DDCMS-Direct-MCP-Server-Backlog.md',
    );
}
