...
function createMcpRouter(fileType: FileTypeLiteral): express.Router {
    ...
    r.post('/generate', async (req: Request, res: Response) => {
        try {
            ...
            const { isValid, errors, normalized, warnings } = validateAndNormalizeMcpRequest(
                fileType,
                body,
            );
            if (!isValid) {
            ...
            if (!['SDDirect', 'EaziPay'].includes(fileType)) {
                return res.status(501).json({
                    ...
                    message: `Generation not yet implemented for ${fileType}`,
                    details: [],
                });
            }
    ...
    r.get('/valid-row', async (req: Request, res: Response) => {
        ...
        const rowCount = Math.max(
            1,
            parseInt(String(req.query.rowCount ?? '1') as string, 10) || 1,
        );
        ...
        const payload = {
            headers,
            rows,
            metadata: { fileType, sun, rowKind: 'valid', generatedAt: new Date().toISOString() },
        };
    ...
    r.get('/invalid-row', async (req: Request, res: Response) => {
        ...
        const payload = {
            headers,
            rows,
            metadata: { fileType, sun, rowKind: 'invalid', generatedAt: new Date().toISOString() },
        };
        return res.status(200).json(payload);
    });
